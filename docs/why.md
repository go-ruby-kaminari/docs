...
`go-ruby-kaminari/kaminari` reimplements Ruby's `kaminari` in **pure Go, with cgo disabled**. The slice of
...
- any Go program can import `github.com/go-ruby-kaminari/kaminari` directly, with no Ruby runtime;
...
Because `kaminari` is pure Go with cgo disabled, it also compiles to **WebAssembly**
